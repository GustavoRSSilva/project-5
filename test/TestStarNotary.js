...
it('can add the star name and star symbol properly', async() => {
    ...
    let tokenId = 6;
    let instance = await StarNotary.deployed();
    await instance.createStar('Star1', tokenId, {from: accounts[0]})
    assert.equal(await instance.tokenIdToStarInfo.call(tokenId), 'Star1')
    assert.equal(await instance.name.call(), 'star')
    assert.equal(await instance.symbol.call(), 'GTS')
});
